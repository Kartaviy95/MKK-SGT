...
private _markers = (GVAR(markerGroups) get "bot") select 0;
{deleteMarker _x} forEach _markers;
_markers = [];

{
    if (!(_x in playableUnits) && {!(_x isKindOf "Animal")}) then {
        private _marker = createMarker [format [QGVAR(bot_%1), count _markers], getPos _x];
...
GVAR(markerGroups) set ["bot", [_markers, true]];
