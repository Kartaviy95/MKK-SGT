...
private _markers = (GVAR(markerGroups) get "build") select 0;
...

{
    if (
        ...
        private _icon = createMarker [format [QGVAR(build_%1), count _markers], getPos _x];
        ...
        private _rect = createMarkerLocal [format [QGVAR(buildBox_%1), count _markers], getPos _x];
...
GVAR(markerGroups) set ["build", [_markers, true]];
